...
export const TROOP_TYPES = ['raider', 'archer', 'lancer', 'ram', 'scaler', 'bomber'] as const;
...
export interface TroopDef {
   ...
   * Flies.
   *
   * The whole air layer hangs off this one flag, and it is worth being precise
   * about what it buys. A flyer does not path: no rampart stops it, no wall
   * turns it, and it goes to its target in a straight line over everything in
   * between. That makes a wall — the thing a defender spends most of their gold
   * on across the whole game — completely irrelevant to it.
   *
   * The counterweight is that most of a base cannot touch it. A Cannon points
   * along the ground and a Mortar lobs a shell at it; neither can reach up.
   * Only the Archer Tower and the Air Defence can, which is what makes "have I
   * covered the air here" a second question every corner of a layout has to
   * answer, on top of the one it already had.
   */
  fly?: boolean;
  /**
   * Can shoot at something flying.
   *
   * Defending troops use this too, so a clan's donated Archers are worth
   * something against a Bomber and their donated Lancers are not.
   */
  air?: boolean;
...
export const TROOP: Record<TroopType, TroopDef> = {
  ...
  // The one troop in the warband that shoots up. That makes an Archer the
  // answer to a defending clan's Bomber, and makes donated Archers worth
  // something a donated Lancer is not.
  archer: { n: 'Archer', hp: 95,  dmg: 21, cd: 0.70, spd: 2.10, rng: 3.40, sp: 2, cost: { g: 60,  i: 55  }, tt: 9,  pref: 'any',  air: true, col: '#8fd67f', col2: '#4a8a44' },
  ...
  /*
   * The Bomber. TUNABLE, and the piece the whole air layer is built round.
   *
   * Slow, tough and expensive, and it goes for buildings rather than defences,
   * because a flyer that beelines for the guns would answer the layout question
   * for the player. It has to drift over the base picking off whatever is
   * nearest so that *where the Air Defence sits* is what decides the raid.
   *
   * Six warband slots, the same as a Ram: a Bomber army is a real commitment
   * and a base with no air cover should lose to it, which is the point.
   */
  bomber: { n: 'Bomber', hp: 620, dmg: 84, cd: 1.60, spd: 1.35, rng: 1.20, sp: 6, cost: { g: 180, i: 210 }, tt: 26, pref: 'any',  fly: true, col: '#d8b26a', col2: '#8a6a2c' },
};

/** Barracks level required before a type can be trained. */
export const TROOP_UNLOCK: Record<TroopType, number> = {
  raider: 1, archer: 1, lancer: 2, ram: 3, scaler: 4, bomber: 5,
};
...
/** Flies over everything, including ramparts. */
export function flies(type: TroopType): boolean {
  return TROOP[type].fly === true;
}

/** Can shoot at something that flies. */
export function hitsAir(type: TroopType): boolean {
  return TROOP[type].air === true;
}
